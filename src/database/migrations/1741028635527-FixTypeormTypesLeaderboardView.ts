...
export class FixTypeormTypesLeaderboardView1741028635527 implements MigrationInterface {
    name = 'FixTypeormTypesLeaderboardView1741028635527'
...
select
    ...
    (sum(count(1)) over (partition by vp.steam_id, vp.season_id))::int as games,
    (sum(count(1) filter (where pim.team = fm.winner)) over (partition by vp.steam_id, vp.season_id))::int as wins,
    vp.mmr as mmr,
